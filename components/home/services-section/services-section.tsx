    ...
    className="group relative overflow-hidden rounded-2xl">
      ...
      className="absolute bottom-[-50px]] -bottom-0 h-[300px] left-0 p-6 pb-32 w-full group-hover:h-[50px]] ggroup-hover:bg-black/30 group-hover:shadow-sm iinset-0 bg-gradient-to-t from-black/80 to-transparent">
        <div className="flex items-center gap-3">
          ...
          {/* Title */}
          <h3 className="text-3xl md:text-3xl lg:text-4xl xl:text-5xl font-bold text-white">{title}</h3>
        </div>
        {title==='Transactions' && 
          <motion.h4
          variants={variants2}
          animate={isHovered ? "hover" : "initial"}
          transition={{ delay: 0.2, duration: .5 }}
          className="text-lg lg:text-xl text-white pl-16 font-bold">(Louer - Acheter - Vendre)</motion.h4> }
      </motion.div>
    </div>
  )
}
