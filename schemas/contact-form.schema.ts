...
export const contactFormSchema = z.object({
    ...
    // Emplacement
    location: z.string().min(2, "L'emplacement doit contenir au moins 2 caractères").max(100, "L'emplacement ne peut pas dépasser 100 caractères"),

    // Superficie et pièces
    surface: z.number().min(1, "La superficie doit être supérieure à 0"),
    rooms: z.number().min(1, "Le nombre de pièces doit être supérieur à 0"),

    // Budget
    minBudget: z.number().min(0, "Le budget minimum ne peut pas être négatif"),
    maxBudget: z.number().min(0, "Le budget maximum ne peut pas être négatif"),

    // Message
    message: z.string().min(10, "Le message doit contenir au moins 10 caractères").max(5000, "Le message ne peut pas dépasser 5000 caractères").optional(),
})